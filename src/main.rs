...
fn main() {
    ...
    let input_one_string = &inputs_one.to_string();
    let input_two_string = &inputs_two.to_string();

    let wire_one_instructions = parse_inputs(input_one_string);
    let wire_two_instructions = parse_inputs(input_two_string);

    let wires_drawn = vec![
        run_instructions(&wire_one_instructions),
        run_instructions(&wire_two_instructions),
    ];

    let wire_one = wires_drawn[0].keys().collect::<HashSet<_>>();
    let wire_two = wires_drawn[1].keys().collect::<HashSet<_>>();
    ...
    let min_intersection_steps_coordinate = wire_one
        .intersection(&wire_two)
        .min_by_key(|p| wires_drawn[0][p] + wires_drawn[1][p])
        .unwrap();

    let wire_one_min_distance = x + y;
    let min_steps = wires_drawn[0][min_intersection_steps_coordinate]
        + wires_drawn[1][min_intersection_steps_coordinate];
    println!(
        "Min distance {:?}, min steps: {:?}",
        wire_one_min_distance, min_steps
    );
}

fn run_instructions(instructions: &Vec<&str>) -> HashMap<(i32, i32), i32> {
    ...
    for i in instructions.iter() {
        ...
        let (dx, dy) = ds[&i.chars().nth(0).unwrap()];
        for _ in 0..val {
            steps += 1;
            x += dx;
            y += dy;

            coordinates.insert((x, y), steps);
        }
    }
    coordinates
}

fn parse_inputs(inputs: &String) -> Vec<&str> {
    inputs.split(",").collect::<Vec<&str>>()
}
